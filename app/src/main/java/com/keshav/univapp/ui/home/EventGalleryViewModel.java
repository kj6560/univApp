...
public class EventGalleryViewModel extends ViewModel {
    // TODO: Implement the ViewModel
    public MutableLiveData<String> mTemp;
    public MutableLiveData<Event> mEvent;
    public MutableLiveData<List<String>> mGallery;

    public LiveData<List<String>> getEventGallery(Context context, int event_id){
        if(mGallery == null){
            mGallery = new MutableLiveData<>();
            fetchEventGallery(context,event_id);
        }
        return mGallery;
    }
                    ...
                    try {
                        if (response.code() == 200) {
                            ...
                            for (int i=0;i<jar.length();i++){
                                ...
                                Log.d("gallery image",image);
                            }

                            mGallery.postValue(eventGallery);
                        }
                    } catch (Exception e) {
